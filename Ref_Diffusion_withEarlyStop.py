...
REFERENCE_IMAGE_PATH = r"C:\Users\ykarr\Desktop\model_Diffusion_gen\Data\train\sequence_0\sequence_0.tif"
REFERENCE_CONTEXT_PATH = r"C:\Users\ykarr\Desktop\model_Diffusion_gen\Data\train\sequence_0\context.pt"
...
class DynamicConfig:
    def __init__(self, data_specs):
        ...
        self.output_dir = self.data_root / "Ref_Diff_early"
...
class DiffusionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # NEW: Load reference image as in Code 2
            with rasterio.open(REFERENCE_IMAGE_PATH) as src_ref:
                ...
                img_ref = torch.tensor(img_ref_data.astype(np.float32))
            ...
            # NEW: Load reference context
            context_ref = torch.load(REFERENCE_CONTEXT_PATH, map_location="cpu")
            ...
            # NEW: Compute context difference
            ...
            if (epoch + 1) % 5 == 0:
                ...
                with torch.no_grad():
                    for x0, context in val_loader:
                        ...
                        # NEW: Add reference image to reconstruct original
                        with rasterio.open(REFERENCE_IMAGE_PATH) as src_ref:
                            img_ref = torch.tensor(src_ref.read()).float().to(device)
                        pred_x0_original = pred_x0_original + img_ref.unsqueeze(0)
...
def generate_samples(model, config, num_samples=1, context=None, num_steps=None, use_ddim=True):
    ...
    with rasterio.open(REFERENCE_IMAGE_PATH) as src_ref:
        img_ref = torch.tensor(src_ref.read()).float().to(device)
    generated = generated_diff + img_ref.unsqueeze(0)
...
def test_model(config, model_path, context_path, num_samples=1):
    try:
        ...
        # NEW: Load reference context
        context_ref = torch.load(REFERENCE_CONTEXT_PATH, map_location=device)
